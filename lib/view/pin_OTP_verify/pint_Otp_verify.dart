...
class pin_OtpVerification extends StatefulWidget {
  const pin_OtpVerification({Key? key}) : super(key: key);

  @override
  State<pin_OtpVerification> createState() => _pin_OtpVerificationState();
}

class _pin_OtpVerificationState extends State<pin_OtpVerification> {

  final GlobalKey<FormState> _key = GlobalKey<FormState>();
  final _otpcontroller = TextEditingController();
  final _keyscafflod = GlobalKey<ScaffoldState>();
  pin_OTP_Notifier? pin_otp_notifier;
  ...
  void setpin(){
    screenLock<void>(
      ...
      didConfirmed: (pass){
        print(pass);
        pin_otp_notifier?.pin_otp_verfiy(email, _otpcontroller.text.trim(),pass);
  ...
  Widget build(BuildContext context) {
    if (!kReleaseMode) {}
    return Consumer<pin_OTP_Notifier>(
  ...
  Widget buttonRequestOTP(context) {
    pin_otp_notifier =
        Provider.of<pin_OTP_Notifier?>(context, listen: false);
    return Center(
      child: SizedBox(
        height: 50,
        width: 234,
        child: ElevatedButton(
          ...
          onPressed: () {
            setpin();
            // pin_otp_notifier?.pin_otp_verfiy(
   ...
   pin_emila_send_OTP_Notfier pin_otp_notfier =
    ...
    return Center(
      child: InkWell(
        ...
        onTap: (){
          pin_otp_notfier.PinEmail_Otp(email);
        },
      ),
    );
  }
