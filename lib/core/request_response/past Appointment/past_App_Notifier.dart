...
import 'package:user_clinic_token_app/core/request_response/past%20Appointment/past_App_Response.dart';
...
class Past_App_Notifier extends BaseChangeNotifier {
  final SharedPref _prefs = locator.get<SharedPref>();
  bool isloading = false;

  List<past_App> past_Appointment_ResponseClass = [];
  List<past_App> get Past_Appointment_Class => past_Appointment_ResponseClass;

  set   Past_Appointment_Class(List<past_App> value) {
    past_Appointment_ResponseClass = value;
    notifyListeners();
  }
  Future<void> Past_App_data(String id) async {
    isLoading = true;
    final prefs = await SharedPreferences.getInstance();
    String url = DataConstants.LIVE_BASE_URL + DataConstants.Past_appointment;
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        final appointmentModel = pastAppointmentFromJson(response.body);
        Past_Appointment_Class = appointmentModel.data!;
        notifyListeners();
