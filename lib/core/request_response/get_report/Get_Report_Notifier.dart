...
class GEt_Report_Notifier extends BaseChangeNotifier {
  final SharedPref _prefs = locator.get<SharedPref>();
  bool isloading = false;

  List<Get_Report> get_report_ResponseClass = [];
  List<Get_Report> get Get_Report_Class => get_report_ResponseClass;

  set   Get_Report_Class(List<Get_Report> value) {
    get_report_ResponseClass = value;
    notifyListeners();
  }

  Future<void> Get_Report_data(String id) async {
    isLoading = true;
    final prefs = await SharedPreferences.getInstance();
    String url = DataConstants.LIVE_BASE_URL + DataConstants.Get_Report;
    print(url);
    try {
      ...
      if (response.statusCode == 200) {
        final GetReportmodel = getReportFromJson(response.body);
        Get_Report_Class=GetReportmodel.data!;
        print(Get_Report_Class.length);
        notifyListeners();
      }
