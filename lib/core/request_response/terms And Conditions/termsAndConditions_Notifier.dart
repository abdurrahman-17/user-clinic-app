...
import 'package:user_clinic_token_app/core/request_response/terms%20And%20Conditions/termsAndConditionsResponse.dart';
...
class TermsAndCondition_Notifier extends BaseChangeNotifier {
  ...
  TermsConditionContent termsAndConditions_ResponseClass = TermsConditionContent();
  TermsConditionContent get TermsAndConditions_Class => termsAndConditions_ResponseClass;

  set   TermsAndConditions_Class(TermsConditionContent value) {
    termsAndConditions_ResponseClass = value;
    notifyListeners();
  }

  Future<void> TermsAndConditions_data() async {
    isLoading = true;
    final prefs = await SharedPreferences.getInstance();
    String url = DataConstants.LIVE_BASE_URL + DataConstants.Terms_And_Conditions;
    print(url);
     try {
    //   final input = {'patient_id': id,'date':date};
      ...
      if (response.statusCode == 200) {
        final appointmentModel = termsAndConditionsFromJson(response.body);
        TermsAndConditions_Class = appointmentModel.termsConditionContent!;
